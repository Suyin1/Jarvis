...
// 导航回调函数指针（静态），由 mainwindow 启动时通过 SetNavigateCallback 注入
TCSimpleHandler::NavigateCallback TCSimpleHandler::navigate_callback_ = nullptr;
...
bool TCSimpleHandler::ExecuteCallbackFunc(const CefString &function_name, const CefString &params, int js_callback_id,
    ...
    if (function_name == "showMsg") {
        ...
        LOG(INFO) << "call API: " << __FUNCTION__ << " navigateToNative params: " << params;
        if (navigate_callback_) {
            navigate_callback_(params.ToString(), js_callback_id, browser, frame);
        } else {
            LOG(WARNING) << "navigate_callback_ not set, cannot navigate to: " << params;
        }
    }
