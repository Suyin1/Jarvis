...
// CEF 渲染进程代理 — 管理渲染线程中的 V8 上下文和 JavaScript 执行
    ...
    virtual void OnWebKitInitialized(CefRefPtr<TCSimpleRender> app) override {
		...
		std::string app_code = R"(
						...
						(() => {
							...
							ClientAPI.navigateToNative = function(pageName, callback) {
								native function navigateToNative(pageName, callback);
								return navigateToNative(pageName, callback);
							};						
						})();
					)";
        if (!m_pV8handler) {
