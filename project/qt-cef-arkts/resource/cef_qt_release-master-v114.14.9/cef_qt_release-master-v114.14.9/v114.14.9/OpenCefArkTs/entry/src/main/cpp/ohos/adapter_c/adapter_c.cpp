...
#include "cef_bridge/CefBridge.h"
...
// NAPI 导出：初始化 CEF 引擎（由 AbilityStage.onCreate 调用）
// 注册桥接回调并启动 CEF 消息循环线程
static napi_value CefInit(napi_env env, napi_callback_info info) {
    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "EWAdapterC", "CefInit: starting CEF engine");
    CefBridge *bridge = CefBridge::GetInstance();
    bridge->RegisterBridges();
    bridge->StartCef();
    return nullptr;
}

// NAPI 导出：通知 C++ 层 XComponent Surface 已就绪（由 Index.ets onLoad 调用）
// 参数1: id — XComponent 的 ID（字符串）
// 参数2: context — onLoad 回调传入的 XComponent context（SURFACE 模式下的原生 surface 上下文）
// 功能：从 XComponent context 中提取 OHNativeWindow 句柄，传递给 CefBridge 创建浏览器
static napi_value CefSurfaceReady(napi_env env, napi_callback_info info) {
    size_t argc = 2;
    napi_value args[2];
    napi_get_cb_info(env, info, &argc, args, nullptr, nullptr);
    ...
    if (argc < 2) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "EWAdapterC",
                     "CefSurfaceReady: requires 2 args (id, context)");
        return nullptr;
    }
    ...
    if (idType != napi_string) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "EWAdapterC",
                     "CefSurfaceReady: arg[0] must be string (id)");
        return nullptr;
    }
    napi_get_value_string_utf8(env, args[0], id, sizeof(id), &idLen);

    // 参数2: XComponent onLoad context（SURFACE 模式的原生 surface context）
    napi_valuetype ctxType;
    napi_typeof(env, args[1], &ctxType);
    if (ctxType != napi_object) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "EWAdapterC",
                     "CefSurfaceReady: arg[1] must be object (XComponent context)");
        return nullptr;
    }

    // 从 onLoad context 中提取 OH_NativeXComponent
    OH_NativeXComponent *nativeXComponent = nullptr;
    napi_status status = OH_NativeXComponent_GetNativeXComponent(env, args[1], &nativeXComponent);
    if (status != napi_ok || nativeXComponent == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "EWAdapterC",
                     "CefSurfaceReady: OH_NativeXComponent_GetNativeXComponent failed, status=%{public}d", status);
        return nullptr;
    }

    // 存储 OH_NativeXComponent（供输入事件等后续使用）
    EWAdapterC::getInstance()->setNativeXComponent(std::string(id), nativeXComponent);

    // 获取原生窗口句柄 OHNativeWindow
    OHNativeWindow *nativeWindow = nullptr;
    status = OH_NativeXComponent_GetNativeWindow(nativeXComponent, &nativeWindow);
    if (status != napi_ok || nativeWindow == nullptr) {
        OH_LOG_Print(LOG_APP, LOG_ERROR, LOG_PRINT_DOMAIN, "EWAdapterC",
                     "CefSurfaceReady: OH_NativeXComponent_GetNativeWindow failed, status=%{public}d", status);
        return nullptr;
    }

    OH_LOG_Print(LOG_APP, LOG_INFO, LOG_PRINT_DOMAIN, "EWAdapterC",
                 "CefSurfaceReady: nativeWindow=%{public}p, id=%{public}s", nativeWindow, id);

    // 通知 CefBridge Surface 已就绪，创建 CEF 浏览器
    CefBridge *bridge = CefBridge::GetInstance();
    bridge->OnSurfaceReady(reinterpret_cast<uint64_t>(nativeWindow));
    return nullptr;
}
...
EXTERN_C_START static napi_value InitAdapter(napi_env env, napi_value exports) {
  ...
  napi_property_descriptor desc[] = {
        ...
        {"cefInit", 0, CefInit, 0, 0, 0, napi_default, 0},
        {"cefSurfaceReady", 0, CefSurfaceReady, 0, 0, 0, napi_default, 0}};
